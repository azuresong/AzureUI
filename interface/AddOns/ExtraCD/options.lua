...
function mod:AddSortingOption(order, spellId)
	local name, _, icon = GetSpellInfo(spellId)
	local dbKey = tostring(spellId)
	...
	op[dbKey] = {
		...
		name = name,
		icon = icon,
		order = function() return db.order end,
		args = {
			moveUp = {
				order = 42,
				name = L["Move up"],
				...
				args = {
					...
					autoload = {
						...
						args = {
							...
							enchantppm = {
								name = L["enchant(ppm)"],
								desc = L["Scan and show an average cd based on haste * real-ppm of player's enchant with a real-ppm mechanism. "],
								type = "toggle",
								order = 201,
								disabled = function() return not self.db.enchant end,
							},
						},
					},
					...
					overlayAtProc = {
						name = L["Glow only at procs"],
						desc = L["Only show glow during procs, otherwise also show it after cooldown ending."],
						type = "toggle",
						order = 313,
						disabled = function() return not self.db.showglow end,
					},
					rowmax = {
