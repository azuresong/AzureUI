...
function HPetBattleAny.ShowMaxValue(petstate,speciesID,point)
	...
	if type(petstate) == "table" then
		...
		rarity = select(5,C_PetJournal.GetPetStats(petstate)) or nil
	end
...
function HPetBattleAny:GetPetCollectedInfo(pets,enemypet,islink,mini)
	...
	if pets then
		...
		for i,petInfo in pairs(pets) do
			...
			if petlink then
				local breedID = select(4,HPetDate.GetBreedValue(petInfo.petID))
				if (breedID and ((breedID>=4 and breedID<=6) or (breedID>=14 and breedID<=16))) then
					petlink=petlink..ICON_LIST[ICON_TAG_LIST[strlower(RAID_TARGET_1)]] .. "0|t"
				end
		...
		if (C_PetBattles.IsPlayerNPC(2) and select(2,C_PetBattles.IsTrapAvailable())>6 and select(2,C_PetBattles.IsTrapAvailable())<9) or select(2,C_PetBattles.IsTrapAvailable())==6 then
...
----- 输出敌对宠物的数据
		...
		for petIndex=1, C_PetBattles.GetNumPets(petOwner) do
			...
			local rarity=C_PetBattles.GetBreedQuality(petOwner,petIndex);if rarity>3 and rarity~=6 and C_PetBattles.IsWildBattle() then self.EnemyPetInfo.FindBlue=true end;
...
function HPetBattleAny:PET_JOURNAL_LIST_UPDATE()
	if (HPetSaves.Contrast or true ) and not self.HasPetloading and PetJournal and GetTime() - 1.5 > Steptime then
		...
		printt("|cffff0000系统数据:"..numPets.."插件数据"..(self.HasPet.num or 0).."|r")
...
function HPetBattleAny:LoadUserPetInfo()
	...
	local MAXnum = select(2,C_PetJournal.GetNumPets(true))
	printt("test:数据处理.开始"..MAXnum)
