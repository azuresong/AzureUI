...
function HPetOption:InitButtons()
	self.Buttons={
		...
		{name="UpdateStone",type="Button",inherits="UIPanelButtonTemplate",
			point="BOTTOMRIGHT",x=-5,y=5,width=100, height=20, text = "升级石",
...
--~ 	if UpdateStoneButtons.ready then UpdateStoneButtons:Toggle() end
	if not BrotherBags then
		if GetLocale() == "zhCN" then
			...
			print(format("error:".."Can't find BrotherBags",1,0,1))
		end
		return
	end
		...
		if Max ~= 0 then
			...
			for rname,t in pairs(StringTable) do
				for pname,c in pairs(t) do
					if UnitName("player") == pname then pname = "|cff00ffff"..pname.."|r" end
					Tpstring = Tpstring..pname..":|cffff00ff"..c.."|r;"
				end
			end
			print(GetItemLink(SearchId),TOTAL..":","|cffffff00"..Max.."|r")
			print(Tpstring)
		end
	end
