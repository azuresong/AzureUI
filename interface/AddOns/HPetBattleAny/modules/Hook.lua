...
-------收集信息(界面提示)
local function GetOwnedInfo(owned,speciesID)
	local str
	if  HPetBattleAny.HasPet[speciesID] then
		for _,petInfo in pairs(HPetBattleAny.HasPet[speciesID]) do
			...
			if str then
				str=petlink..format("|c"..RAID_CLASS_COLORS["ROGUE"].colorStr.."%s|r",petInfo.level).."/"..str
			else
				str=petlink..format("|c"..RAID_CLASS_COLORS["ROGUE"].colorStr.."%s|r",petInfo.level)
			end
		end
...
hookfunction.init = function()
	for a,b in pairs(HPetBattleAny.hook) do
		...
		elseif a=="PJSTooltip" then
			for i = 1, 12 do
				_G["PetJournalListScrollFrameButton"..i].dragButton:HookScript("OnEnter",b)
				_G["PetJournalListScrollFrameButton"..i].dragButton:HookScript("OnLeave",GameTooltip_Hide)
--~ 				_G["PetJournalListScrollFrameButton"..i]:SetScript("OnEnter",b)
--~ 				_G["PetJournalListScrollFrameButton"..i]:SetScript("OnLeave",GameTooltip_Hide)
			end
			for i = 1, 3 do
				_G["PetJournalLoadoutPet"..i].dragButton:HookScript("OnEnter",b)
...
hookfunction.PetJournal_UpdatePetCard=function(self)
	...
	if PetJournalPetCard.petID then
		...
		if breedID then
			self.HealthFrame.health:SetText(format("%d%s",self.HealthFrame.health:GetText(),ghealth))
			self.PowerFrame.power:SetText(format("%d%s",self.PowerFrame.power:GetText(),gpower))
			self.SpeedFrame.speed:SetText(format("%d%s",self.SpeedFrame.speed:GetText(),gspeed))
...
hookfunction.PJSTooltip = function(self)
...
--~ 			if HPetAllInfoFrame.ready then HPetAllInfoFrame:Update(speciesID,HPetBattleAny.EnemyPetInfo[self.petIndex].breedID) end
			if HPetAllInfoFrame then
				if not HPetAllInfoFrame.ready then HPetAllInfoFrame:Toggle() end
				HPetAllInfoFrame:Update(speciesID,HPetBattleAny.EnemyPetInfo[self.petIndex].breedID)
...
hookfunction.StaticPopup_Show=function(str)
	if (str=="PET_BATTLE_FORFEIT" or str=="PET_BATTLE_FORFEIT_NO_PENALTY") and HPetSaves.FastForfeit then
		if HPetBattleAny.EnemyPetInfo and HPetBattleAny.EnemyPetInfo.FindBlue then
		else
			StaticPopup_Hide("PET_BATTLE_FORFEIT",nil);
...
---------------------------自动保存设置宠物技能组合
local SaveSkillInfo = function(loadoutID,index,abilityID,stop)
...
local LoadSkillInfo = function(loadoutID,petID,stop)
	...
	for i = 1,3 do
		...
		if abilityID and abilityID~=_G["aID"..i] then
