	...
	for _,slot in pairs(slots) do
		...
		if itemLink then
			...
			if source then
				...
				local reforgeId = nil
				reforgeId = ReforgingInfo:GetReforgeID(itemLink)
				itemsFound = true
				...
				cat.hasItems = true
				cat.items[cat.count] = {link = itemLink, enchant = enchantId, reforged = reforgeId, slot = slot}
			end
...
function IE:AddItems(tab, padding)
	for _, item in pairs(tab) do
		...
		if item.reforged and (item.reforged ~= 0) then
			suffix = suffix .. " |cff00cc00[R]|r"
		end
