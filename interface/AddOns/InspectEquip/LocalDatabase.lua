	...
	for tier = 1, tierCount do
		...
		while true do
			...
			while insID do
				...
				local diffOffset = (mop and isRaid) and 2 or 0
				local maxDiff = isRaid and 5 or 2
				-- EJ_IsValidInstanceDifficulty and EJ_SetDifficulty use different values in 5.x
				-- unfortunately the constants are local variables in the EJ implementation
				-- this code works with 5.0.4 and 4.3
				for diff = 1, maxDiff do
					if EJ_IsValidInstanceDifficulty(diff + diffOffset) then
						EJ_SetDifficulty(diff)
