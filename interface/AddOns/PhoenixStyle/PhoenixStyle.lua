...
pslocations={{752,754,758,773,800,824},{77777,897,896,886,930}}
pslocationnamesdef={{"Baradin Hold","Blackwing Descent","The Bastion of Twilight","Throne of the Four Winds","Firelands", "Dragon Soul"},{"New Baradin Hold","Heart of Fear","Mogu'shan Vaults","Terrace of Endless Spring","Throne of Thunder"}}
...
if IsAddOnLoaded("PhoenixStyle_Loader_Cata") then
  psoldcatawipebossid(id)
    ...
    if (id==68065 or id==70212 or id==70235 or id==70247) then
      psstrazhfdf=0 --головы мегеры не тригерят конец боя!
    end
...
--если 6 переменных то обнуляем, временно убрать потом 3 строчки ыытест, как забуду что это значит - тупо удалить!
if ps_saoptions and #ps_saoptions==6 then
  ps_saoptions=nil
end
...
--ыытест временно, забыл? удалил это:
ps_saoptions[2][3][3][1]=nil
...
local atext="|cffff0000Important!|r\n\nGarrosh, Sargeras or even Old Gods. New dungeons and epic achievements. |cff00ff00PhoenixStyle|r doesn’t want to |cffff0000abandon you|r in the face of dangerous adventures. Do you want |cff00ff00to learn more – visit|r our site!\n\n|cff00ff00Highlight link and click  Ctrl+C to copy|r"
  if GetLocale()=="ruRU" then
 ...
 if GetLocale()=="zhCN" then
    atext="|cffff0000重要!|r\n\n加尔鲁什,萨格拉斯甚至是上古之神.新的地下城和史诗成就.|cff00ff00PhoenixStyle|r并不想在面对危险的冒险时|cffff0000抛弃你|r.你想要|cff00ff00了解更多–请访问|r我们的网站!\n\n|cff00ff00选中高亮显示连接并按Ctrl+C 来复制|r"
  end
...
for i=1,#pslocations do
  for j=1,#pslocations[i] do
    if GetCurrentMapAreaID()==pslocations[i][j] then
      if IsAddOnLoaded(psaddontoload[i][j])==nil and pswasmoduletryloaded[i][j]==0 then
...
if IsAddOnLoaded(psaddontoload[pssetexpans][psmenuchoose1])==nil then
...
--print ("ыы запись босса после боя в инфо")
...
if (psdonaspanvar==nil or (psdonaspanvar and (psdonaspanvar==1 or psdonaspanvar==2) and int and string.find(int,"5.2"))) then
local a1=math.random(1,4)
if a1==4 and UnitInRaid("player")==nil and UnitInParty("player")==nil then
  --сообщение
  local text="Garrosh, Sargeras or even Old Gods. New dungeons and epic achievements. |cff00ff00PhoenixStyle|r doesn’t want to |cffff0000abandon you|r in the face of dangerous adventures. Do you want |cff00ff00to learn more – visit|r http://www.phoenixstyle.com/help.php"
  if GetLocale()=="ruRU" then
    text="Гаррош, Саргерас или Древние Боги. Новые данжи и новые ачивки. |cff00ff00PhoenixStyle|r не хочет |cffff0000бросать вас|r перед лицом опасных испытаний. Хотите |cff00ff00узнать больше|r – http://www.phoenixstyle.com/help.php"
  end
  if GetLocale()=="itIT" then
    text="|cff00ff00Messaggio importante|r. Il progetto |cff00ff00PhoenixStyle|r forse sarà |cffff0000chiuso|r, per sappere cosa si può fare - http://www.phoenixstyle.com/help.php"
  end
  ...
  if psdonaspanvar==nil then
    psdonaspanvar=1
  elseif psdonaspanvar and (psdonaspanvar==1 or psdonaspanvar==2) then
    psdonaspanvar=3 --2ка используется для аннонса РА после 20 декабря и после 1 января, только если 1 ВКЛ! иначе же РА сразу прыгает с 1 до 3
  end
else
  psnotproched=1
end
...
function psshowdoateinf(nr)
  if psnotshowfirstopen and psnotshowfirstopen>5 and fdsffoNOTREPORTNOW then


pscurrentnrview=nr
--проверка времени отображения
local _, month, day, year = CalendarGetDate()
local int=GetBuildInfo()
if (((month==12 and day>20) or month<12) and psshowaddframe34==nil and nr==2) or (psshowaddframe33==nil and int and string.find(int,"5.2") and nr==1) then
  PSF_closeallpr()
  PSFemptyframe:Show()
  ...
  local txt1="Dear friends, |cff00ff00PhoenixStyle|r is with us for a lot of contents, |cff00ff00almost 3 years!|r During this time it united both casual players and hardcore raiders. Doing some achievements in heroics at the Sunday evening or planning a strategy with your guild for the new raid boss – PhoenixStyle will always give you a helpful hand due to its |cff00ff00unique functions and mods.|r\n\n\nNow you can |cff00ff00track the donation|r activity in real time (updates every 12h).\n\n\n|cff00ff00Click  Ctrl+C  to copy|r"
  local txt2="and |cff00ff00continue|r with addon..."
  if GetLocale()=="ruRU" then
    txt1="Друзья мои. |cff00ff00PhoenixStyle|r с нами уже на протяжении многих контентов, |cff00ff00а точнее 3 года!|r За это время он объединил как простых игроков, так и хардкор рейдеров. Хотите ли вы субботним вечером поделать ачивки в героиках, или продумываете как улучшить страту на нового босса – феникс стайл всегда готов прийти вам на помощь, благодаря своим |cff00ff00уникальным функциям и модам.|r\n\n\nТеперь на сайте отображается |cff00ff00статус пожертвований|r (обновление раз в 12ч).\n\n\n|cff00ff00Нажмите  Ctrl+C  чтобы скопировать|r"
...
adfsdfsdfjy4:SetFocus()
  ...
  if nr==1 then
    ...
    if psnotshowfirstopen==nil then
      psnotshowfirstopen=0
    end
    psnotshowfirstopen=psnotshowfirstopen+1
    if nr==1 then
      PSF_buttonraids()
    else
      psbutclickedsavedinfo()
