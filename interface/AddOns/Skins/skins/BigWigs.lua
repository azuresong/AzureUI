...
local applystyle = function(bar)
	...
	if bar.candyBarIconFrame:GetTexture() then
		...
		ibg:SetParent(bar)
		ibg:ClearAllPoints()
		ibg:SetPoint("TOPLEFT", bar.candyBarIconFrame, "TOPLEFT", 1, -1)
		ibg:SetPoint("BOTTOMRIGHT", bar.candyBarIconFrame, "BOTTOMRIGHT", -1, 1)
		ibg:SetFrameStrata("BACKGROUND")
		ibg:Show()
		bar:Set("bigwigs:bigwigs:bg", ibg)
	end
	-- setup timer and bar name fonts and positions
	bar.candyBarLabel:SetFont("Fonts\\Frizqt__.ttf", 12, "OUTLINE")
	bar.candyBarLabel:ClearAllPoints()
	bar.candyBarLabel:SetPoint("BOTTOMLEFT", bar, "TOPLEFT", -2, 4)

	bar.candyBarDuration:SetFont("Fonts\\Frizqt__.ttf", 12, "OUTLINE")
	...
	bar.candyBarBar:SetStatusBarTexture("Interface\\addons\\skins\\statusbar4")
	...
	bar.candyBarBackground:SetTexture("Interface\\addons\\skins\\statusbar4")
...
local function registerStyle()
	...
	if bars then
		bars:RegisterBarStyle("BigWigs", {
			apiVersion = 1,
			version = 1,
			GetSpacing = function(bar) return buttonsize+3 end,
