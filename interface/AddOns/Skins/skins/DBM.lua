				...
				if not bar.injected then
					bar.ApplyStyle = function()
						...
						if not icon1.styled then
							...
							icon1:SetPoint("TOPLEFT", icon1.overlay, 0, -0)
							icon1:SetPoint("BOTTOMRIGHT", icon1.overlay, -0, 0)
							icon1.styled = true
						end
						...
						if not icon2.styled then
							...
							icon2:SetPoint("TOPLEFT", icon2.overlay, 0, -0)
							icon2:SetPoint("BOTTOMRIGHT", icon2.overlay, -0, 0)
							icon2.styled = true
						end
						
						if not texture.styled then
							texture:SetTexture("Interface\\addons\\skins\\statusbar4")
							texture.styled = true
						end
						
						if not tbar.styled then
							tbar:SetPoint("TOPLEFT", frame, "TOPLEFT", 0, -0)
							tbar:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -0, 0)
							tbar.styled = true
						end
		...
		local SkinBossTitle = function()
			...
			if not anchor.styled then
				...
				if header[1]:IsObjectType("FontString") then
					header[1]:SetFont("Fonts\\Frizqt__.ttf", 10, "OUTLINE")
		...
		local SkinBoss = function()
			...
			while (_G[format("DBM_BossHealth_Bar_%d", count)]) do
				...
				if not progress.styled then
					progress:SetStatusBarTexture("Interface\\addons\\skins\\statusbar4")
