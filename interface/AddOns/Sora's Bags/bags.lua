...
function MyButton:OnCreate()
	...
	self.Count:SetFont("Fonts\\Hooge.ttf", 9, "OUTLINE MONOCHROME") --( DB.Font, 10, "THINOUTLINE")
	
	self.Border = CreateFrame("Frame", nil, self)
	self.Border:SetAllPoints(self.Icon)
	self.Border:SetBackdrop({edgeFile = DB.Solid, edgeSize = 1.2})
...
function MyContainer:OnCreate(name, settings)
	...
	tagDisplay:SetFont("Fonts\\Hooge.ttf", 9, "OUTLINE MONOCHROME") --(DB.Font, 12)
	...
	searchText:SetFont(DB.Font, 13, "THINOUTLINE")
	...
	BagToggle.Text:SetFont(DB.Font, 13, "THINOUTLINE")
	...
	SortButton:SetScript("OnMouseDown", function(_,btn) if btn=="LeftButton" then JPack:Pack(nil,1) else JPack:Pack(nil,2) end end)
	SortButton.Text = SortButton:CreateFontString(nil, "OVERLAY")
	SortButton.Text:SetPoint("CENTER")
	SortButton.Text:SetFont(DB.Font, 13, "THINOUTLINE")
