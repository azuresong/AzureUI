...
local createBackdrop2 = function(parent, anchor) 
    local frame = CreateFrame("Frame", nil, parent)
    frame:SetFrameStrata("LOW")
    ...
    if pixelborder then
        ...
        frame:SetPoint("TOPLEFT", anchor, "TOPLEFT", -4, 2)
        frame:SetPoint("BOTTOMRIGHT", anchor, "BOTTOMRIGHT", 4, -2)
        frame:SetBackdrop(frameBD)
    end
...
local castbar = function(self, unit)
	...
	if multicheck(u, "target", "focus", "boss", "player") then
		
		local cb = createStatusbar(self, texture, "OVERLAY", 20, width-26, 1, .25, .35, .5)
		...
		elseif(unit=="target")then
			...
			cb.Icon:SetPoint("BOTTOMLEFT", cb, "BOTTOMRIGHT", 6, 0)
...
local func = function(self, unit)
	...
	self.FrameBackdrop = createBackdrop2(self, self)
	...
	if(unit == "targettarget" or unit == "focustarget" or unit == "focus") then
		hp:SetHeight(height-4)
	else
		hp:SetHeight((height-4)*hpheight)
	end
	...
	if (unit=="pet")then
		local pname = createFont(hp, "OVERLAY", font, fontsize, fontflag, 1, 1, 1)
		self:Tag(pname,'[freeb:info] [freeb:name]')
		pname:SetPoint("left",hp,4,7)
		--pname:SetJustifyH"LEFT"
	elseif not (unit == "targettarget" or unit == "focustarget") then
        local hpp = createFont(hp, "OVERLAY", font, fontsize, fontflag, 1, 1, 1)
        if(unit == "player") then
            self:Tag(hpp, '[freeb:hp]')
			hpp:SetPoint("right", hp, -2, 7)

       else
            self:Tag(hpp, '[freeb:hp]')
			hpp:SetPoint("left", hp, 2, -10)
        end
    end
    ...
    if not (unit == "targettarget" or unit == "focustarget" or unit == "focus") then
        local pp = createStatusbar(self, texture, nil, (height-4)*-(hpheight-1)-1, nil, 1, 1, 1, 1)
        pp:SetPoint"LEFT"
        pp:SetPoint"RIGHT"
        pp:SetPoint("BOTTOM",0,2)

        pp.frequentUpdates = 0.05
        pp.Smooth = true
    ...
    if(unit == "targettarget" or unit == "focustarget") then
        name:SetPoint("LEFT", hp,0,6)
        name:SetPoint("RIGHT", hp,0,6)
 ...
 --[[       name:SetPoint("LEFT", hp, 2, 0)
        ...
        if(unit == "player") then
				name.frequentUpdates = 0.05
				self:Tag(name, '[freeb:pp]')
				name:SetPoint("left",hp,2,7)
        elseif(unit=="pet")then
				name.frequentUpdates = 0.05
				self:Tag(name, '[freeb:pp]')
				name:SetPoint("right",hp,-4,7)
				name:SetJustifyH"RIGHT"
		elseif classColorbars then
            self:Tag(name, '[freeb:info] [freeb:name]')
			name:SetPoint("LEFT", hp, 2, 7)
			name:SetPoint("RIGHT",hp,-2,7)
			name:SetJustifyH"LEFT"
		else
	...
	if(unit=="pet" or unit == "targettarget" or unit == "focustarget" or unit == "focus") then
...
local UnitSpecific = {
	...
	target = function(self, ...)
		...
		if portraits then
            ...
            self.Portrait:SetWidth(55)
            self.Portrait:SetHeight(45)
            self.Portrait:SetPoint("TOPRIGHT", self, "BOTTOMRIGHT", 0, -5)
            self.PorBackdrop = createBackdrop(self, self.Portrait)
        end
...
oUF:Factory(function(self)
	spawnHelper(self, "player", "bottomright",UIParent,"bottom", -85, 300)
    spawnHelper(self, "target", "bottomleft",UIParent,"bottom", 85, 300)
	spawnHelper(self, "targettarget", "LEFT","oUF_FreebTarget","RIGHT", 10, 0)
	spawnHelper(self, "focus", "TOPRIGHT",UIParent,"center", -100, 150)
	spawnHelper(self, "focustarget", "RIGHT", "oUF_FreebFocus", "LEFT", -10, 0)
	spawnHelper(self, "pet", "RIGHT", "oUF_FreebPlayer", "LEFT", -10, 0)

	if bossframes then
		for i = 1, MAX_BOSS_FRAMES do
			spawnHelper(self,'boss' .. i, "CENTER", 450, 180 - (60 * i))
		end
	end
end)
